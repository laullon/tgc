...
function tgc_historias_usuario() {
    $user = wp_get_current_user();
    $user_id = $user->ID;
    $posts = get_posts(array("author" => $user_id, "orderby" => 'ID',"numberposts"=>-1));
    foreach ($posts as $post) {
        $res[]=array('ID' => $post->ID,'titulo'=>get_the_title($post->ID));
    }
    return $res;
}
...
function tgc_lista_historias_usuario() {
    if (is_user_logged_in ()) {
        ...
        $posts = get_posts(array("author" => $user_id, "orderby" => 'ID'));
